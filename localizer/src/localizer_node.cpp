...
struct NodeConfig
{
    ...
    double max_hard_pose_jump = 2.5;
    double max_hard_yaw_jump = 45.0 * 3.14159265358979323846 / 180.0;
    ...
    bool enable_tf_velocity_guard = true;
    double max_tf_correction_speed = 2.0;
    double max_tf_correction_yaw_rate = 15.0 * 3.14159265358979323846 / 180.0;
    bool enable_motion_gate = false;
    double max_alignment_speed = 1.5;
    double max_alignment_yaw_rate = 120.0 * 3.14159265358979323846 / 180.0;
    double max_tf_correction_step = 0.25;
    double max_tf_correction_yaw_step = 5.0 * 3.14159265358979323846 / 180.0;
    double max_tf_correction_accept = 1.0;
    double max_tf_correction_yaw_accept = 12.0 * 3.14159265358979323846 / 180.0;
    double velocity_history_sec = 3.0;
    double velocity_recovery_age_sec = 0.7;
    double velocity_recovery_cooldown_sec = 2.0;
    bool enable_pose_hold_on_lost = true;
    int pose_hold_reject_count = 20;
    bool enable_auto_fresh_relocalize = false;
    int auto_fresh_relocalize_reject_count = 12;
    double auto_fresh_relocalize_cooldown_sec = 3.0;
};
...
struct NodeState
{
    ...
    bool recovery_guess_received = false;
    bool localize_success = false;
    double ignore_messages_before_sec = 0.0;
    rclcpp::Time last_send_tf_time = rclcpp::Clock().now();
    rclcpp::Time last_map_cloud_pub_time = rclcpp::Time(0, 0, RCL_ROS_TIME);
    bool map_cloud_published = false;
    ...
    double current_local_speed = 0.0;
    double current_local_yaw_rate = 0.0;
    ...
    bool has_trusted_relocalize_pose = false;
    double last_good_stamp = 0.0;
    M3D trusted_relocalize_r = M3D::Identity();
    V3D trusted_relocalize_t = V3D::Zero();
    ...
    double auto_fresh_relocalize_cooldown_until = 0.0;
    bool pose_hold_active = false;
    M3D held_pose_r = M3D::Identity();
    V3D held_pose_t = V3D::Zero();
};
...
public:
    LocalizerNode() : Node("localizer_node")
    {
        ...
        m_fastlio_reset_client = this->create_client<std_srvs::srv::Trigger>("/fastlio2/reset");
    ...
    void loadParameters()
    {
        ...
        m_config.max_hard_pose_jump = config["max_hard_pose_jump"] ? config["max_hard_pose_jump"].as<double>() : 2.5;
        double max_hard_yaw_jump_deg = config["max_hard_yaw_jump_deg"] ? config["max_hard_yaw_jump_deg"].as<double>() : 45.0;
        m_config.max_hard_yaw_jump = max_hard_yaw_jump_deg * 3.14159265358979323846 / 180.0;
        ...
        m_config.enable_tf_velocity_guard = config["enable_tf_velocity_guard"] ? config["enable_tf_velocity_guard"].as<bool>() : true;
        m_config.max_tf_correction_speed = config["max_tf_correction_speed"] ? config["max_tf_correction_speed"].as<double>() : 2.0;
        double max_tf_correction_yaw_rate_deg = config["max_tf_correction_yaw_rate_deg"] ? config["max_tf_correction_yaw_rate_deg"].as<double>() : 15.0;
        m_config.max_tf_correction_yaw_rate = max_tf_correction_yaw_rate_deg * 3.14159265358979323846 / 180.0;
        m_config.enable_motion_gate = config["enable_motion_gate"] ? config["enable_motion_gate"].as<bool>() : false;
        m_config.max_alignment_speed = config["max_alignment_speed"] ? config["max_alignment_speed"].as<double>() : 1.5;
        double max_alignment_yaw_rate_deg = config["max_alignment_yaw_rate_deg"] ? config["max_alignment_yaw_rate_deg"].as<double>() : 120.0;
        m_config.max_alignment_yaw_rate = max_alignment_yaw_rate_deg * 3.14159265358979323846 / 180.0;
        m_config.max_tf_correction_step = config["max_tf_correction_step"] ? config["max_tf_correction_step"].as<double>() : 0.25;
        double max_tf_correction_yaw_step_deg = config["max_tf_correction_yaw_step_deg"] ? config["max_tf_correction_yaw_step_deg"].as<double>() : 5.0;
        m_config.max_tf_correction_yaw_step = max_tf_correction_yaw_step_deg * 3.14159265358979323846 / 180.0;
        m_config.max_tf_correction_accept = config["max_tf_correction_accept"] ? config["max_tf_correction_accept"].as<double>() : 1.0;
        double max_tf_correction_yaw_accept_deg = config["max_tf_correction_yaw_accept_deg"] ? config["max_tf_correction_yaw_accept_deg"].as<double>() : 12.0;
        m_config.max_tf_correction_yaw_accept = max_tf_correction_yaw_accept_deg * 3.14159265358979323846 / 180.0;
        ...
        m_config.enable_pose_hold_on_lost = config["enable_pose_hold_on_lost"] ? config["enable_pose_hold_on_lost"].as<bool>() : true;
        m_config.pose_hold_reject_count = config["pose_hold_reject_count"] ? config["pose_hold_reject_count"].as<int>() : 20;
        m_config.enable_auto_fresh_relocalize = config["enable_auto_fresh_relocalize"] ? config["enable_auto_fresh_relocalize"].as<bool>() : false;
        m_config.auto_fresh_relocalize_reject_count = config["auto_fresh_relocalize_reject_count"] ? config["auto_fresh_relocalize_reject_count"].as<int>() : 12;
        m_config.auto_fresh_relocalize_cooldown_sec = config["auto_fresh_relocalize_cooldown_sec"] ? config["auto_fresh_relocalize_cooldown_sec"].as<double>() : 3.0;
    ...
    void timerCB()
    {
        ...
        if (!update_tf)
        {
            if (m_state.pose_hold_active)
            {
                ...
                applyPoseHoldOffset(current_local_r, current_local_t);
        ...
        bool using_recovery_guess = false;
        M3D current_local_r;
        V3D current_local_t;
        double current_local_speed = 0.0;
        double current_local_yaw_rate = 0.0;
        builtin_interfaces::msg::Time current_time;
        {
            ...
            current_local_r = m_state.last_r;
            current_local_t = m_state.last_t;
            current_local_speed = m_state.current_local_speed;
            current_local_yaw_rate = m_state.current_local_yaw_rate;
            current_time = m_state.last_message_time;
        }
        ...
        else if (m_state.recovery_guess_received)
        {
            std::lock_guard<std::mutex> lock(m_state.service_mutex);
            initial_guess = m_state.initial_guess;
            m_state.recovery_guess_received = false;
            using_recovery_guess = true;
        }
        else
        {
        ...
        if (m_config.enable_motion_gate && m_state.has_good_pose && !using_service_guess && !using_recovery_guess &&
            isRobotMovingTooFast(current_local_speed, current_local_yaw_rate))
        {
            RCLCPP_WARN_THROTTLE(
                this->get_logger(), *this->get_clock(), 1000,
                "Skip localization alignment while moving fast: speed=%.3fm/s yaw_rate=%.1fdeg/s",
                current_local_speed,
                current_local_yaw_rate * 180.0 / 3.14159265358979323846);
            holdLastGoodPoseIfLost(current_local_r, current_local_t, true);
            sendBroadCastTF(current_time);
        ...
        if (result)
        {
            ...
            bool pose_jump_detected = m_config.enable_pose_guard && m_state.has_good_pose && !using_service_guess && !using_recovery_guess &&
                                      isPoseJump(initial_guess, predicted_pose, translation_jump, yaw_jump);
            bool tf_jump_detected = m_config.enable_pose_guard && m_state.has_good_pose && !using_service_guess && !using_recovery_guess &&
                                    isTfJump(proposed_offset_r, proposed_offset_t, tf_translation_jump, tf_yaw_jump);
            double hard_translation_jump = 0.0;
            double hard_yaw_jump = 0.0;
            bool hard_pose_jump_detected = m_config.enable_pose_guard && m_state.has_good_pose && !using_service_guess && !using_recovery_guess &&
                                           isHardPoseJump(map_body_r, map_body_t, hard_translation_jump, hard_yaw_jump);
            if (hard_pose_jump_detected)
            {
                RCLCPP_WARN_THROTTLE(
                    this->get_logger(),
                    *this->get_clock(),
                    1000,
                    "Large candidate offset from last good pose; keep global alignment but limit TF step: translation=%.3fm yaw=%.1fdeg",
                    hard_translation_jump,
                    hard_yaw_jump * 180.0 / 3.14159265358979323846);
                tf_jump_detected = true;
            }
            bool wait_for_stable_tf = false;
            if (pose_jump_detected)
            {
                RCLCPP_WARN_THROTTLE(
                    this->get_logger(), *this->get_clock(), 1000,
                    "Large ICP pose correction from global map alignment; keep alignment but limit TF step: pose_translation=%.3fm pose_yaw=%.1fdeg",
                    translation_jump,
            ...
            if (tf_jump_detected)
            {
                RCLCPP_WARN_THROTTLE(
                    this->get_logger(), *this->get_clock(), 1000,
                    "Large map->local TF correction from global map alignment: tf_translation=%.3fm tf_yaw=%.1fdeg rough_score=%.4f refine_score=%.4f",
                    tf_translation_jump,
                    tf_yaw_jump * 180.0 / 3.14159265358979323846,
                    m_localizer->lastRoughScore(),
                    m_localizer->lastRefineScore());
                if (tf_translation_jump > m_config.max_tf_correction_accept ||
                    tf_yaw_jump > m_config.max_tf_correction_yaw_accept)
                {
                    ++m_state.rejected_alignment_count;
                    RCLCPP_WARN_THROTTLE(
                        this->get_logger(), *this->get_clock(), 1000,
                        "Skip suspicious global map correction: tf_translation=%.3fm/%0.3fm tf_yaw=%.1f/%.1fdeg rejected_count=%d",
                        tf_translation_jump,
                        m_config.max_tf_correction_accept,
                        tf_yaw_jump * 180.0 / 3.14159265358979323846,
                        m_config.max_tf_correction_yaw_accept * 180.0 / 3.14159265358979323846,
                        m_state.rejected_alignment_count);
                    sendBroadCastTF(current_time);
                    publishMapCloud(current_time);
                    return;
                ...
                if (m_state.stable_rejected_candidate_count < m_config.recovery_stable_count)
                {
                    ...
                    RCLCPP_WARN_THROTTLE(
                        this->get_logger(), *this->get_clock(), 1000,
                        "Wait for stable global map correction before applying TF step: tf_translation=%.3fm tf_yaw=%.1fdeg rejected_count=%d stable_candidate_count=%d/%d",
                        tf_translation_jump,
                        tf_yaw_jump * 180.0 / 3.14159265358979323846,
                        m_state.rejected_alignment_count,
                        m_state.stable_rejected_candidate_count,
                        m_config.recovery_stable_count);
                    scheduleFreshRelocalizeIfNeeded(map_body_r, map_body_t, now_sec, using_service_guess, using_recovery_guess);
                ...
                wait_for_stable_tf = true;
                limitOffsetCorrection(proposed_offset_r, proposed_offset_t);
                map_body_r = proposed_offset_r * current_local_r;
                map_body_t = proposed_offset_r * current_local_t + proposed_offset_t;
                RCLCPP_WARN_THROTTLE(
                    this->get_logger(), *this->get_clock(), 1000,
                    "Limit map->local TF correction step: max_translation_step=%.3fm max_yaw_step=%.1fdeg",
                    m_config.max_tf_correction_step,
                    m_config.max_tf_correction_yaw_step * 180.0 / 3.14159265358979323846);
            }
            if (wait_for_stable_tf)
            {
                RCLCPP_WARN_THROTTLE(
                    this->get_logger(), *this->get_clock(), 1000,
                    "Accept stable global map correction after %d matching candidates: pose_translation=%.3fm pose_yaw=%.1fdeg tf_translation=%.3fm tf_yaw=%.1fdeg",
                    m_state.stable_rejected_candidate_count,
                    translation_jump,
                    yaw_jump * 180.0 / 3.14159265358979323846,
                    tf_translation_jump,
            ...
            if (m_config.enable_drift_guard && m_state.has_good_pose && !using_service_guess && !using_recovery_guess)
            {
            ...
            double tf_speed = 0.0;
            double tf_yaw_rate = 0.0;
            if (m_config.enable_tf_velocity_guard && m_state.has_good_pose && !using_service_guess && !using_recovery_guess &&
                isTfVelocityJump(proposed_offset_r, proposed_offset_t, now_sec, tf_speed, tf_yaw_rate))
            {
                RCLCPP_WARN_THROTTLE(
                    this->get_logger(),
                    *this->get_clock(),
                    1000,
                    "Large map->local TF correction velocity from global map alignment: tf_speed=%.3fm/s tf_yaw_rate=%.1fdeg/s",
                    tf_speed,
                    tf_yaw_rate * 180.0 / 3.14159265358979323846);
            }

            double robot_speed = 0.0;
            double robot_yaw_rate = 0.0;
            if (m_config.enable_velocity_guard && m_state.has_good_pose && !using_service_guess && !using_recovery_guess &&
            ...
            m_state.last_good_stamp = now_sec;
            ...
            saveGoodPose(map_body_r, map_body_t, now_sec);
            {
                ...
                if (!m_state.localize_success && m_state.service_received)
                {
                    ...
                    m_state.service_received = false;
                    m_state.has_trusted_relocalize_pose = true;
                    m_state.trusted_relocalize_r = map_body_r;
                    m_state.trusted_relocalize_t = map_body_t;
                    RCLCPP_WARN(
                        this->get_logger(),
                        "Relocalize ICP accepted: x=%.3f y=%.3f z=%.3f yaw=%.1fdeg",
                        map_body_t.x(),
                        map_body_t.y(),
                        map_body_t.z(),
    ...
    void syncCB(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &cloud_msg, const nav_msgs::msg::Odometry::ConstSharedPtr &odom_msg)
    {
        ...
        std::lock_guard<std::mutex> lock(m_state.message_mutex);
        const double cloud_stamp_sec = rclcpp::Time(cloud_msg->header.stamp).seconds();
        if (cloud_stamp_sec <= m_state.ignore_messages_before_sec)
            return;
        ...
        m_state.current_local_speed = std::sqrt(
        ...
        m_state.current_local_yaw_rate = std::abs(odom_msg->twist.twist.angular.z);
    ...
    bool isHardPoseJump(const M3D &candidate_r,
                        const V3D &candidate_t,
                        double &translation_jump,
                        double &yaw_jump) const
    {
        if (m_state.good_pose_history.empty())
            return false;

        const PoseSample &last_good = m_state.good_pose_history.back();
        translation_jump = (candidate_t - last_good.translation).norm();
        M3D delta_r = last_good.rotation.transpose() * candidate_r;
        yaw_jump = std::abs(std::atan2(delta_r(1, 0), delta_r(0, 0)));

        return translation_jump > m_config.max_hard_pose_jump ||
               yaw_jump > m_config.max_hard_yaw_jump;
    }

    bool isRobotMovingTooFast(double speed, double yaw_rate) const
    {
        return speed > m_config.max_alignment_speed ||
               yaw_rate > m_config.max_alignment_yaw_rate;
    }

    void limitOffsetCorrection(M3D &candidate_offset_r, V3D &candidate_offset_t) const
    {
        V3D delta_t = candidate_offset_t - m_state.last_offset_t;
        double delta_norm = delta_t.norm();
        if (delta_norm > m_config.max_tf_correction_step && delta_norm > 1e-6)
        {
            candidate_offset_t = m_state.last_offset_t +
                                 delta_t * (m_config.max_tf_correction_step / delta_norm);
        }

        M3D delta_r = m_state.last_offset_r.transpose() * candidate_offset_r;
        double delta_yaw = std::atan2(delta_r(1, 0), delta_r(0, 0));
        double limited_yaw = std::max(
            -m_config.max_tf_correction_yaw_step,
            std::min(m_config.max_tf_correction_yaw_step, delta_yaw));

        M3D limited_delta_r = Eigen::AngleAxisd(limited_yaw, Eigen::Vector3d::UnitZ()).toRotationMatrix();
        candidate_offset_r = m_state.last_offset_r * limited_delta_r;
    }
    ...
    bool isTfVelocityJump(const M3D &candidate_offset_r,
                          const V3D &candidate_offset_t,
                          double now_sec,
                          double &speed,
                          double &yaw_rate) const
    {
        double dt = now_sec - m_state.last_good_stamp;
        if (dt < 0.05)
            return false;

        speed = (candidate_offset_t - m_state.last_offset_t).norm() / dt;
        M3D delta_r = m_state.last_offset_r.transpose() * candidate_offset_r;
        yaw_rate = std::abs(std::atan2(delta_r(1, 0), delta_r(0, 0))) / dt;

        return speed > m_config.max_tf_correction_speed ||
               yaw_rate > m_config.max_tf_correction_yaw_rate;
    }
    ...
    bool holdLastGoodPoseIfLost(const M3D &current_local_r, const V3D &current_local_t, bool force = false)
    {
        if ((!force && !m_config.enable_pose_hold_on_lost) ||
            (!force && m_state.rejected_alignment_count < m_config.pose_hold_reject_count) ||
            m_state.good_pose_history.empty())
        {
            return false;
        }

        const PoseSample &last_good = m_state.good_pose_history.back();
        m_state.pose_hold_active = true;
        m_state.held_pose_r = last_good.rotation;
        m_state.held_pose_t = last_good.translation;
        applyPoseHoldOffset(current_local_r, current_local_t);
        m_state.correction_history.clear();
        m_state.has_rejected_candidate = false;
        m_state.stable_rejected_candidate_count = 0;
        ...
        RCLCPP_WARN_THROTTLE(
            this->get_logger(), *this->get_clock(), 1000,
            "Hold last good map pose while global map alignment is lost: rejected_count=%d x=%.3f y=%.3f yaw=%.1fdeg",
            m_state.rejected_alignment_count,
            last_good.translation.x(),
            last_good.translation.y(),
            std::atan2(last_good.rotation(1, 0), last_good.rotation(0, 0)) * 180.0 / 3.14159265358979323846);
        return true;
    }

    void applyPoseHoldOffset(const M3D &current_local_r, const V3D &current_local_t)
    {
        m_state.last_offset_r = m_state.held_pose_r * current_local_r.transpose();
        m_state.last_offset_t = -m_state.held_pose_r * current_local_r.transpose() * current_local_t + m_state.held_pose_t;
    }

    void recoverFromStableOffset(const M3D &current_local_r,
                                 ...
                                 double now_sec)
    {
        ...
        M4F stable_guess = M4F::Identity();
        stable_guess.block<3, 3>(0, 0) = (m_state.last_offset_r * current_local_r).cast<float>();
        stable_guess.block<3, 1>(0, 3) = (m_state.last_offset_r * current_local_t + m_state.last_offset_t).cast<float>();

        {
            std::lock_guard<std::mutex> lock(m_state.service_mutex);
            m_state.initial_guess = stable_guess;
            m_state.recovery_guess_received = true;
    ...
    void scheduleFreshRelocalizeIfNeeded(const M3D &fallback_r,
                                         const V3D &fallback_t,
                                         double now_sec,
                                         bool using_service_guess,
                                         bool using_recovery_guess)
    {
        if (!m_config.enable_auto_fresh_relocalize ||
            using_service_guess ||
            using_recovery_guess ||
            m_state.rejected_alignment_count < m_config.auto_fresh_relocalize_reject_count ||
            now_sec < m_state.auto_fresh_relocalize_cooldown_until)
        {
            return;
        }

        M3D fresh_r = fallback_r;
        V3D fresh_t = fallback_t;
        if (m_state.has_trusted_relocalize_pose)
        {
            fresh_r = m_state.trusted_relocalize_r;
            fresh_t = m_state.trusted_relocalize_t;
        }
        else if (!m_state.good_pose_history.empty())
        {
            const PoseSample &last_good = m_state.good_pose_history.back();
            fresh_r = last_good.rotation;
            fresh_t = last_good.translation;
        }

        {
            std::lock_guard<std::mutex> lock(m_state.service_mutex);
            m_state.initial_guess.setIdentity();
            m_state.initial_guess.block<3, 3>(0, 0) = fresh_r.cast<float>();
            m_state.initial_guess.block<3, 1>(0, 3) = fresh_t.cast<float>();
            m_state.recovery_guess_received = true;
            m_state.localize_success = false;
        }
        ...
        m_state.pose_hold_active = false;
        m_state.auto_fresh_relocalize_cooldown_until =
            now_sec + m_config.auto_fresh_relocalize_cooldown_sec;

        RCLCPP_WARN(
            this->get_logger(),
            "Auto fresh relocalize: reset guards and retry ICP from x=%.3f y=%.3f yaw=%.1fdeg cooldown=%.1fs",
            fresh_t.x(),
            fresh_t.y(),
            std::atan2(fresh_r(1, 0), fresh_r(0, 0)) * 180.0 / 3.14159265358979323846,
            m_config.auto_fresh_relocalize_cooldown_sec);
    }
    ...
    void relocCB(const std::shared_ptr<interface::srv::Relocalize::Request> request, std::shared_ptr<interface::srv::Relocalize::Response> response)
    {
        ...
        M3D initial_r = (yaw_angle * roll_angle * pitch_angle).toRotationMatrix();
        V3D initial_t = V3D(x, y, z);
        ...
        }
        {
            std::lock_guard<std::mutex> lock(m_state.service_mutex);
            m_state.initial_guess.setIdentity();
            m_state.initial_guess.block<3, 3>(0, 0) = initial_r.cast<float>();
            m_state.initial_guess.block<3, 1>(0, 3) = initial_t.cast<float>();
            ...
            m_state.has_good_pose = false;
            m_state.has_trusted_relocalize_pose = false;
            m_state.velocity_recovery_cooldown_until = 0.0;
            m_state.auto_fresh_relocalize_cooldown_until = 0.0;
            m_state.last_good_stamp = 0.0;
            m_state.map_cloud_published = false;
            m_state.pose_hold_active = false;
            m_state.has_rejected_candidate = false;
            m_state.stable_rejected_candidate_count = 0;
            m_state.rejected_alignment_count = 0;
        ...
        {
            std::lock_guard<std::mutex> lock(m_state.message_mutex);
            m_state.message_received = false;
            m_state.last_cloud->clear();
            m_state.ignore_messages_before_sec = this->get_clock()->now().seconds();
        }
        requestFastlioReset();
        RCLCPP_WARN(
            this->get_logger(),
            "Relocalize requested: use pose as ICP initial guess x=%.3f y=%.3f z=%.3f yaw=%.1fdeg",
            x,
            y,
            z,
    ...
    void publishMapCloud(builtin_interfaces::msg::Time &time)
    {
        ...
        if (m_state.map_cloud_published &&
            (now - m_state.last_map_cloud_pub_time).seconds() < 5.0)
            return;
        ...
        m_state.map_cloud_published = true;
        m_state.last_map_cloud_pub_time = now;
    }

private:
    void requestFastlioReset()
    {
        if (!m_fastlio_reset_client->service_is_ready())
        {
            RCLCPP_WARN(
                this->get_logger(),
                "FASTLIO reset service is not ready; relocalize will continue without resetting FASTLIO");
            return;
        }

        auto request = std::make_shared<std_srvs::srv::Trigger::Request>();
        m_fastlio_reset_client->async_send_request(
            request,
            [this](rclcpp::Client<std_srvs::srv::Trigger>::SharedFuture future)
            {
                try
                {
                    ...
                    if (response->success)
                    {
                        RCLCPP_WARN(this->get_logger(), "FASTLIO reset accepted: %s", response->message.c_str());
                    }
                    else
                    {
                        RCLCPP_WARN(this->get_logger(), "FASTLIO reset rejected: %s", response->message.c_str());
                    }
                }
                catch (const std::exception &e)
                {
                    RCLCPP_WARN(this->get_logger(), "FASTLIO reset call failed: %s", e.what());
                }
            });
    }
    ...
    rclcpp::Client<std_srvs::srv::Trigger>::SharedPtr m_fastlio_reset_client;
};
